...
run_title_screen! = |prev| {
	...
	draw_animation!(state.rocci_idle_anim, { x: player_x, y: player_start_y_pixel + shift, flags: [] })
...
run_game! = |prev| {
	...
	draw_animation!(state.rocci_flap_anim, { x: player_x, y: y_pixel, flags: [] })
...
run_game_over! = |prev| {
	...
	draw_animation!(state.rocci_fall_anim, { x: player_x, y: y_pixel, flags: [] })
	...
	if state.new_high_score {
		draw_animation!(state.high_score_anim, { x: 64, y: 0, flags: [] })
	}
...
draw_pipe! = |sprite, { x, gap_start }| {
	set_sprite_colors!()
	Sprite.blit!(sprite, { x, y: gap_start - W4.screen_height(), flags: [FlipY] })
	Sprite.blit!(sprite, { x, y: gap_start + gap_height, flags: [] })
}
...
draw_plant! = |sprite_sheet, { x, type }| {
	...
	set_sprite_colors!()
	Sprite.blit!(sprite, { x, y: plant_y, flags: [] })
}
...
draw_ground! = |x| {
	set_ground_colors!()
	Sprite.blit!(ground_sprite, { x, y: W4.screen_height() - 13, flags: [] })
	Sprite.blit!(ground_sprite, { x: x + W4.screen_width(), y: W4.screen_height() - 13, flags: [] })
}
...
draw_animation! : Animation, { x : I32, y : I32, flags : List([FlipX, FlipY, Rotate]) } => {}
