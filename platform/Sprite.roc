...
Sprite := {
	...
	Flags :: { bits : U32 }.{
		...
		flip_x : Flags -> Flags
		flip_x = |{ bits }| { bits: bits.bitwise_or(2) }

		flip_y : Flags -> Flags
		flip_y = |{ bits }| { bits: bits.bitwise_or(4) }

		rotate : Flags -> Flags
		rotate = |{ bits }| { bits: bits.bitwise_or(8) }
	...
	##
	## ```
	## Sprite.blit!(fruit_sprite, { x: 0, y: 0, flags: Sprite.Flags.default().flip_x().rotate() })
	## ```
	##
	...
	blit! : Sprite, { x : I32, y : I32, flags : Flags } => {}
	blit! = |sprite, { x, y, flags }| {
		...
		combined = format + Flags.to_u32(flags)
